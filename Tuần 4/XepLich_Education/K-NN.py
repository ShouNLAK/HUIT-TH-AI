...
# DỮ LIỆU ĐẦU VÀO: EDUCATION
data_source = [
    {
        "MaMon": "CauTrucDuLieu", 
        "ThoiLuongPhut": 120, 
        "DanhSachSV": ["SV011", "SV013", "SV016", "SV027", "SV028"]
    },
    {
        "MaMon": "CoSoDuLieu", 
        "ThoiLuongPhut": 90, 
        "DanhSachSV": ["SV012", "SV015", "SV017", "SV023", "SV024", "SV026"]
    },
    {
        "MaMon": "MangMayTinh", 
        "ThoiLuongPhut": 90, 
        "DanhSachSV": ["SV004", "SV005", "SV013", "SV014", "SV016", "SV023", "SV024", "SV026", "SV028", "SV029", "SV030"]
    },
    {
        "MaMon": "ToanRoiRac", 
        "ThoiLuongPhut": 90, 
        "DanhSachSV": ["SV001", "SV002", "SV003", "SV004", "SV011", "SV012", "SV020", "SV021", "SV024", "SV026", "SV027", "SV028", "SV029"]
    }
]
    ...
    for key in data[0].keys():
        ...
        for item in data:
            ...
            if isinstance(val, list):
                for v in val: val_map[v].append(item[id_key])
            elif isinstance(val, str): 
                val_map[val].append(item[id_key])
                
        for k_val, ids in val_map.items():
            if len(ids) > 1:
                G.add_edges_from(itertools.combinations(ids, 2))
    return G, node_names

def extract_features(G, node_names):
    pos = nx.spring_layout(G, seed=CONFIG["RANDOM_SEED"])
    for n in node_names:
        G.nodes[n]['coords'] = pos[n]
    return np.array([pos[n] for n in node_names])
...
if __name__ == "__main__":
    ...
    labels = np.array([0] * (len(X_features)//2) + [1] * (len(X_features) - len(X_features)//2))
