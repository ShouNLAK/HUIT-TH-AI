            ...
            if (p.isupper() and is_red) or (p.islower() and not is_red):
                ...
                if p_upper == 'R':
                    ...
                    for dr, dc in [(-2,-2), (-2,2), (2,-2), (2,2)]:
                        ...
                        br, bc = r + dr//2, c + dc//2
                        if 0 <= nr < 10 and 0 <= nc < 9:
                            if (is_red and nr < 5) or (not is_red and nr > 4): continue 
...
def minimax_alpha_beta(state, alpha, beta, is_maximizing, indent=0, current_depth=0):
    ...
    if is_maximizing:
        ...
        for action, next_state in successors(state):
            has_moves = True
            eval_score = minimax_alpha_beta(next_state, alpha, beta, False, indent + 1, current_depth + 1)
        ...
        for action, next_state in successors(state):
            has_moves = True
            eval_score = minimax_alpha_beta(next_state, alpha, beta, True, indent + 1, current_depth + 1)
...
def find_best_move(state, initial_alpha, initial_beta):
    ...
    print(f"[AI REPORT] AI ({turn}) suy nghĩ KHÔNG GIỚI HẠN ĐỘ SÂU")
    best_val = -math.inf if is_max else math.inf
        
    for action, next_state in successors(state):
        eval_score = minimax_alpha_beta(next_state, alpha, beta, not is_max, indent=1, current_depth=1)
        ...
        if eval_score <= initial_alpha or eval_score >= initial_beta:
            ...
            eval_score = minimax_alpha_beta(next_state, -math.inf, math.inf, not is_max, indent=1, current_depth=1)
...
def main():
    print("="*40)
    print("CỜ TƯỚNG - MINIMAX ALPHA-BETA KHÔNG DEPTH")
    ...
    while True:
        ...
        if state['turn'] == user_side:
            ...
            move = find_best_move(state, initial_alpha, initial_beta)
            if move:
