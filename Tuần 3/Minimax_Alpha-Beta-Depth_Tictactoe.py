...
def minimax_alpha_beta(state, depth, alpha, beta, is_maximizing, indent=0, current_depth=0):
    ...
    if is_maximizing:
        ...
        for action in successors(state):
            ...
            eval_score = minimax_alpha_beta(state, depth - 1, alpha, beta, False, indent + 1, current_depth + 1)
        ...
        for action in successors(state):
            make_move(state, action)
            eval_score = minimax_alpha_beta(state, depth - 1, alpha, beta, True, indent + 1, current_depth + 1)
            undo_move(state, action)
...
def find_best_move(state, depth, initial_alpha, initial_beta):
    ...
    if is_max:
        best_val = -math.inf
        print(f"\n[AI REPORT] AI (X - MAX) suy nghĩ với Depth = {depth}. Alpha={alpha}, Beta={beta}")
    else:
        best_val = math.inf
        print(f"\n[AI REPORT] AI (O - MIN) suy nghĩ với Depth = {depth}. Alpha={alpha}, Beta={beta}")
        
    for action in successors(state):
        print(f"\n-> Phân tích nhánh ngoài cùng: Thử {action}:")
        make_move(state, action)
        eval_score = minimax_alpha_beta(state, depth - 1, alpha, beta, not is_max, indent=1, current_depth=1)
        undo_move(state, action)
...
def main():
    print("="*50)
    print("TIC TAC TOE - ALPHA-BETA CÓ DEPTH (GLOBAL STATE 1D)")
    print("="*50)
    ...
    while True:
        ...
        if current_state['turn'] == user_side:
            ...
            move_idx = find_best_move(current_state, depth, initial_alpha, initial_beta)
